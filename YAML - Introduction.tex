...
\newcommand{\Subject}{Introductory Presentation about\texorpdfstring{\\}{}
                      the Serialization Language YAML}
...
\begin{frame}[fragile]{🌊 Flow Style}
  \begin{enumerate}
    ...
    \begin{itemize}
      ...
      \item Indicators (\code{:}, \code{[}, …) only under certain circumstances as first character
    \end{itemize}
...
\begin{frame}[fragile, allowframebreaks]{🗒 Block Style}
  ...
  \begin{block}{Block Scalar Header}
    \begin{enumerate}
      ...
      \end{itemize}
    \end{enumerate}
    Arbitrary combinations \codebox{\texttt{|-8}}, \code{>4+}, … possible
  \end{block}
